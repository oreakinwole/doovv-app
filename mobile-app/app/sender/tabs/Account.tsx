...
const Account = () => {
    ...
    // Handle screen focus to manage map state
    useFocusEffect(
        React.useCallback(() => {
    ...
    const getProfileImageUri = () => {
        return profileImage || userData?.imageUrl || null;
    };

    const pickImage = async () => {
        try {
            // Temporarily disable screen focus to pause map rendering
            setIsScreenFocused(false);
            ...
            // Re-enable screen focus after image picker closes
            setTimeout(() => setIsScreenFocused(true), 100);
    ...
    return (
        <LayoutPage noscroll={true} classname={'p-4 pt-[13%]'}>
            <View style={tw`items-center mt-8 mb-6`}>
                {renderProfileImage()}
                <Text style={tw`text-2xl font-bold mt-4`}>{userData?.fullname || 'User'}</Text>
            ...
            <FullPageModalLayout 
                modalVisible={personalInfo} 
                setModalVisible={setPersonalInfo} 
                button={() => setPersonalInfo(false)}
            >
                ...
                <View style={tw`px-4`}>
                    <View style={tw`flex-row items-center py-4 border-b border-gray-200`}>
                        ...
                        <Text style={tw`text-lg font-medium`}>{userData?.fullname || 'Not available'}</Text>
                    </View>

                    <View style={tw`flex-row items-center py-4 border-b border-gray-200`}>
                        <Feather name="smartphone" size={20} color="black" style={tw`mr-4`} />
                        <Text style={tw`text-lg font-medium`}>{userData?.phone || 'Not available'}</Text>
                    </View>

                    <View style={tw`flex-row items-center py-4 border-b border-gray-200`}>
                        <MaterialIcons name="email" size={20} color="black" style={tw`mr-4`} />
                        <Text style={tw`text-lg font-medium`}>{userData?.email || 'Not available'}</Text>
                    </View>
                </View>
            </FullPageModalLayout>

            <FullPageModalLayout 
                modalVisible={switchRole} 
                setModalVisible={setSwitchRole} 
                button={() => setSwitchRole(false)}
            >
                <View style={tw`p-4`}>
                    ...
                    <SelectionCard
                        ...
                        icon={<VehicleImg />}
                        handleClick={() => setSwitchRole(false)}
